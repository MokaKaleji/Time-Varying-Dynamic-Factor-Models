...
% visualize results.
% Explanation: This script loads estimation outputs from QMLDFM_TVL, 
% generates forecasts for a user-specified horizon H, computes error metrics
% (MSFE, RMSE), compares against benchmarks (random walk, AR(1)), conducts
% statistical  tests (Diebold-Mariano, forecast encompassing, Ljung-Box), 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Purpose: Generate forecasts using the DFTL model estimates.
...
% smoothed states (xitT), companion matrix (Ahat), loadings (Lhat), and 
% covariances (Sigma_e_hat, Qhat). The deterministic option 
% (stochastic=false) is used for point forecasts.
[MK_Forecast]=DFTL_Forecast(MK.xitT, MK.Ahat, MK.Lhat, MK.Sigma_e_hat, ...
    MK.Qhat, H, R, p, mean_train, std_train, false);
...
squared_errors = (x_test(:, key_vars) - MK_Forecast.yhat(:, key_vars)).^2;
...
% Explanation: Regresses the forecast error (y - f_DFTL) on the difference
% (f_benchmark - f_DFTL). A non-significant coefficient suggests DFTL 
...
for k = 1:length(key_vars)
    ...
    f0  = MK_Forecast.yhat(:,   key_vars(k));                              % DFTL forecast
...
residuals = x_test(:, key_vars) - MK_Forecast.yhat(:, key_vars);
...
max_lags = 10;
[cross_corr, lags] = xcorr(MK.Fhat(:,i), MK.Fhat(:,j), max_lags, 'coeff');
...
for idx = 1:4
    ...
    plot(T_train+1:T_train+H, x_test(:, key_vars(idx)), 'k-', 'DisplayName', ...
        'Test Actual');
    plot(T_train+1:T_train+H, MK_Forecast.yhat(:, key_vars(idx)), 'r--', ...
...
subplot(3,4,7);
plot(MK.Fhat, 'LineWidth', 1.5);
title('Estimated Factors over Time'); xlabel('Time'); ylabel('Value');
legend(arrayfun(@(k)['Factor ' num2str(k)], 1:size(MK.Fhat,2), ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [MK_Forecast] = DFTL_Forecast(xitT, Ahat, Lhat, ...
    Sigma_e_hat, Qhat, H, R, p, mean_train, std_train, stochastic)
    ...
    % (Lambda_t) but time-invariant VAR coefficients (Ahat), with forecasts
    % either deterministic or stochastic.
    % Inputs:
    %   xitT: T x (R*(p+1)) smoothed states from QMLDFM_TVL
    ...
    % Outputs: MK_Forecast (struct containing):
    ...
    if size(Sigma_e_hat) ~= [N, N]
        error('Sigma_e_hat must be %d x %d, got %d x %d', N, N, size ...
            (Sigma_e_hat, 1), size(Sigma_e_hat, 2));
    end
    if H < 1
    ...
    % for forecasting.
    state = xitT(T, 1:state_dim)';                                         % state_dim x 1, e.g., 10 x 1
    ...
    if stochastic
        ...
        try
            ...
            warning(['Qhat(1:R, 1:R) is not positive definite. Adding small' ...
                ' diagonal term.']);
        ...
        try
            ...
            warning(['Sigma_e_hat is not positive definite. Adding small ' ...
                'diagonal term.']);
    ...
    MK_Forecast.yhat = X_forecast .* std_train + mean_train;
    MK_Forecast.yhat_norm=X_forecast;
    MK_Forecast.Fhat_forecast=Fhat_forecast;
end