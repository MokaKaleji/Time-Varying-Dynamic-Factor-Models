...
% with time-varying loadings, A, and Q (DFTLTATQ), evaluate forecast accuracy,
% and visualize results.
% Explanation: This script loads estimation outputs from QMLDFM_TVLAQ, generates
...
load('dftl&A&Q_estim_results.mat');
...
[MKAQ_Forecast] = DFTLTATQ_Forecast(MKAQ.xitT, MKAQ.Ahat_companion, MKAQ.Lhat, ...
    MKAQ.Sigma_e_hat, MKAQ.Qhat_aug, H, R, p, mean_train, std_train, false);
...
squared_errors = (x_test(:, key_vars) - MKAQ_Forecast.yhat(:, key_vars)).^2;
...
% last training observation. MSFE ratios (DFTLTATQ vs. RW) indicate relative 
% performance.
...
% forecasts H steps ahead. MSFE ratios (DFTLTATQ vs. AR(1)) assess DFTLTA’s 
...
% Purpose: Test whether DFTLTATQ forecasts are significantly more accurate than
% benchmarks.
...
% Explanation: Regresses the forecast error (y - f_DFTLTATQ) on the difference
% (f_benchmark - f_DFTLTATQ). A non-significant coefficient suggests DFTLTATQ 
...
for k = 1:length(key_vars)
    ...
    f0  = MKAQ_Forecast.yhat(:,   key_vars(k));                            % DFTL forecast
    ...
    fprintf(' DFTLTATQ vs RW:  \tβ̂=%.3f, t=%.2f, p=%.3f\n', beta_rw(2), t_rw, ...
        p_rw);
    fprintf(' DFTLTATQ vs AR1: \tβ̂=%.3f, t=%.2f, p=%.3f\n', beta_a1(2), t_a1, ...
        p_a1);
end
...
residuals = x_test(:, key_vars) - MKAQ_Forecast.yhat(:, key_vars);
...
% ratios, aiding interpretation of DFTLTATQ performance and factor dynamics.
...
max_lags = 10;
[cross_corr, lags] = xcorr(MKAQ.Fhat(:,i), MKAQ.Fhat(:,j), max_lags, 'coeff');
...
for idx = 1:3
    ...
    plot(1:T_train, MKAQ.CChat(1:T_train, key_vars(idx)), 'r--', 'DisplayName', ...
        'Estimate'); hold on;
    plot(T_train+1:T_train+H, x_test(:, key_vars(idx)), 'k-', 'DisplayName', ...
        'Test Actual');
    plot(T_train+1:T_train+H, MKAQ_Forecast.yhat(:, key_vars(idx)), 'g-', ...
...
fig7 = figure;
plot(MKAQ.Fhat, 'LineWidth', 1.5);
title('Estimated Factors over Time'); xlabel('Time'); ylabel('Value');
legend(arrayfun(@(k)['Factor ' num2str(k)], 1:size(MKAQ.Fhat,2), ...
...
title('MSFE Ratios: DFTLTATQ vs RW and AR(1)');
xticklabels(var_names); ylabel('Ratio');
legend('DFTLTATQ / RW', 'DFTLTATQ / AR(1)', 'Location', 'Best'); grid on;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [MKAQ_Forecast] = DFTLTATQ_Forecast(xitT, Ahat, Lhat, Sigma_e_hat, ...
    ...
    % outputs from QMLDFM_TVLAQ. The model assumes time-varying loadings 
    % (Lambda_t), transition matrix (Ahat), and Q with forecasts either 
    ...
    %   Qhat: (R*p) x (R*p) factor innovation covariance at time T
    %   H: Forecast horizon
    %   R: Number of factors
    %   p: Number of lags
...
for h = 1:H
    ...
    MKAQ_Forecast.yhat_norm=X_forecast;
    MKAQ_Forecast.yhat=X_forecast .* std_train + mean_train;
    MKAQ_Forecast.Fhat_forecast=Fhat_forecast;
end
